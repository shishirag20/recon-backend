...
4. For each candidate bank row, in order:
   - **Phase 0 (`INTAKE_VALIDATION`)** — `dup-utr` runs first, in its own pre-pass ahead of customer identification entirely: if the reference collides (this run or a prior one), the row gets rejected outright — a `DUPLICATE` exception, no `payments` row at all, `bank_statements.recon_status → EXCEPTION` — and Phase 1a/1b never run for it at all.
   - **Phase 1a (`CUSTOMER_LOCK`)** — only reached if Phase 0 didn't reject. Try each enabled rule in ascending `priority`, stop at the first one that fires: `expected-utr` → `account-ifsc` → `upi` → `customer-code` → `gstin-pan` → `fuzzy-name` try in turn; the first exact (or, for the last one, high-similarity) hit **locks** the row: one `payments` row is inserted with `customer_id` set and `locked_by_rule_id` recording which rule won.
   - **Phase 1b (`CANDIDATE_POOL`)** — only reached if every 1a rule missed. `account-suffix` then `narration-tokens` try in turn, first-match-wins on *whether a rule finds anything*, but the rule that fires returns *every* customer it matches, not just one — so it can genuinely return two candidates (e.g. two customers sharing the same last-4-digit account suffix). This always writes `candidate_pool` (a JSON array of customer IDs) and leaves `customer_id` `NULL`; Phase 1b never locks, by design.
...
3. Within the scoped invoice set, rules try in priority order, first-match-wins per payment: `exact-invoice-num` (the full invoice number appears verbatim in narration) → `invoice-suffix` (only the last 4+ digits appear) → `exact-amount` (the payment exactly equals one open invoice's `balance_due_minor` — if it ties between two invoices with the same balance, this deliberately does *not* guess; it raises an Ambiguous/Scoped exception instead, e.g. the golden test's Coral Living pair) → `tds-match` (payment equals balance minus that invoice's `allowed_tds_minor`) → `subset-sum` (search combinations of up to 10 open invoices, oldest due date first, for one that sums exactly to the payment — a single payment can settle several invoices at once) → `bank-fee` (the shortfall exactly equals the bank row's own `explicit_fee_minor` — the fee is decoupled from the invoice rather than counted as a partial payment) → `write-off` (a residual within a small materiality threshold, e.g. ₹5, gets written off instead of left open forever) → `overpay-on-account` (payment exceeds the invoice — the excess becomes unapplied/on-account credit, not an error) → `partial-pay` (the universal fallback: whatever no other rule claimed becomes a straightforward partial payment).
...
1. `gl_posting.post_run()` runs as the last step of `engine.run()`, inside the same transaction as Phase 1/2 — never speculatively mid-resolution, and never as a separate worker pass. It posts one journal entry **per bank_txn** (not per allocation), built from `run_phase_1`'s `suspense_records` and `run_phase_2`'s `payment_ledger_records` — both are plain in-memory handoffs, the same "pass the outcome forward" pattern Phase 1→Phase 2 already uses, since there's no `run_id` column on `payments`/`invoice_allocations` to re-query this by later. Four shapes, covering every outcome a payment can reach:
   ...
   - **A gap** (TDS withheld, a decoupled bank fee, or a small write-off) — `AR_CONTROL` is credited for the invoice's *full* closed amount, `CASH_CONTROL` only for the cash actually received, and the difference debits whichever role the firing rule maps to (`GAP_ROLE_BY_RULE_KIND`: `tds-match → TDS_RECEIVABLE`, `bank-fee → BANK_CHARGES`, `write-off → WRITE_OFF`).
...
| `rules/identification.py` | ✅ Done | `dup_utr_check` (Phase 0, own pre-pass) + the 6 Phase 1a rules, `IDENTIFICATION_RULES` registry |
...
- **No ingestion path derives `invoices.allowed_tds_minor` or `tds_rate_pct` from a source file either** (e.g. `SL.csv`'s `tds_allowed_pct` column has no `field_mappings` synonym today). `tds-match` (M2) works around this by computing the effective TDS amount itself from `tds_rate_pct` at match time - but `tds_rate_pct` still has to get into the DB somehow first, which today means direct seeding, not a real upload.
...
- **`bank-fee` and `write-off` share the same default tolerance (500 minor units) with no other differentiator**, and `bank-fee` evaluates first (priority 60 vs 70). For a residual within tolerance on a bank row with no `explicit_fee_minor` set, `bank-fee`'s generic fallback always wins - `write-off` can only ever fire if `bank-fee` is disabled or its tolerance is tightened below the residual. Found via the golden dataset's BANK-014/INV-118 case (named "WRITE OFF TEST PAYMENT" in the fixture, but actually resolves via `bank-fee`, posting its gap to `BANK_CHARGES` rather than `WRITE_OFF` - see `test_golden_m3.py::test_small_residual_118_gap_posts_to_bank_charges_not_write_off`). Not fixed here since it's a rule-catalog tuning question (should `write-off`'s default threshold be smaller than `bank-fee`'s, or should priority order flip?), not a code bug.
