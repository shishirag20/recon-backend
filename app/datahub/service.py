...
class DataHubService:
    ...
    async def create_data_source(self, *, entity_id: str, name: str, kind: str):
        ...
        return await self.dao.insert_data_source(entity_id=entity_id, name=name, kind=kind)

    async def get_data_source(self, source_id: str):
        row = await self.dao.get_data_source(source_id)
        if row is None:
            raise HTTPException(status.HTTP_404_NOT_FOUND, DataHubErrors.DATA_SOURCE_NOT_FOUND)
        return row
    ...
    async def update_data_source(self, source_id: str, *, name: str | None, status_: str | None):
    ...
    async def preview_mapping(self, source_id: str, sample_rows: list[dict], mappings_override: list[dict] | None):
        await self.get_data_source(source_id)
        mappings = mappings_override if mappings_override is not None else await self.dao.get_active_mappings(source_id)
        if not mappings:
            raise HTTPException(status.HTTP_400_BAD_REQUEST, DataHubErrors.NO_ACTIVE_MAPPING)
        results = []
    ...
    async def create_upload_job(
        self, *, source_id: str, stream: str, fmt: str, file: UploadFile, started_by: str | None
    ):
        ...
        if fmt not in SUPPORTED_UPLOAD_FORMATS:
            raise HTTPException(status.HTTP_415_UNSUPPORTED_MEDIA_TYPE, DataHubErrors.UNSUPPORTED_FORMAT)
        await self.get_data_source(source_id)  # 404s if missing
        ...
        return await self.dao.insert_ingest_job(
            job_id=job_id, source_id=source_id, stream=stream,
            file_name=safe_filename, file_uri=dest_path, fmt=fmt, started_by=started_by,
        )

    async def _save_upload(self, file: UploadFile, dest_dir: str, dest_path: str) -> None:
        ...
        try:
            while True:
                ...
                if total > MAX_UPLOAD_BYTES:
                    raise HTTPException(status.HTTP_413_REQUEST_ENTITY_TOO_LARGE, DataHubErrors.FILE_TOO_LARGE)
