...
RuleFn = Callable[
    [dict, dict, str, AllocationContext, dict], Awaitable[AllocationOutcome]
]
...
async def _promote_unresolved_invoice(
    ctx: AllocationContext, inv: dict, customer_id: str
) -> None:
...
def _matched_number(narration: str, inv: dict) -> str | None:
    ...
    if inv.get("document_number") and extract.contains_substring(
...
def tds_adjusted_close(amount: int, inv: dict) -> tuple[int, str] | None:
    ...
    if net_amount == amount:
        return (
            computed_tds,
            f"amount matches balance net of {_rupees(computed_tds)} TDS",
        )
    return None
...
def resolve_dust_threshold(config: dict) -> int:
    ...
    if threshold is None:
        val = (
            config.get("max_writeoff_amount")
            or config.get("materiality_threshold")
            or 500
        )
...
def resolve_invoice_settlement(
    amount: int, inv: dict, bank_txn: dict
) -> SettlementClose:
    ...
    if tds is not None:
        _, reason = tds
        return SettlementClose(
            "VARIANCE", amount, True, reason, GAP_ROLE_BY_RULE_KIND["tds-match"]
        )
    ...
    if explicit_fee > 0:
        ...
        if reason is not None:
            return SettlementClose(
                "VARIANCE", amount, True, reason, GAP_ROLE_BY_RULE_KIND["bank-fee"]
            )

    reason = dust_writeoff_close(amount, inv, resolve_dust_threshold({}))
    if reason is not None:
        return SettlementClose(
            "VARIANCE", amount, True, reason, GAP_ROLE_BY_RULE_KIND["write-off"]
        )
    ...
    if amount > balance:
        ...
        return SettlementClose(
            ...
            balance,
            True,
            f"invoice fully settled, {excess_fmt} excess on-account",
        )
...
def _settled_outcome(
    inv: dict, amount: int, bank_txn: dict, label: str
) -> AllocationOutcome:
    ...
    reason = (
        label if settle.status in ("EXACT", "PARTIAL") else f"{label} - {settle.reason}"
    )
    match_type = (
        "EXACT"
...
async def invoice_number_match(
    payment: dict,
    bank_txn: dict,
    customer_id: str,
...
async def truncated_suffix_match(
    payment: dict,
    bank_txn: dict,
    customer_id: str,
    ctx: AllocationContext,
...
async def exact_balance_match(
    ...
    closes = [
        (inv, resolve_invoice_settlement(amount, inv, bank_txn))
        for inv in _open_invoices(ctx, customer_id)
    ]
    ...
    if overpaid:
        inv, settle = min(
            overpaid, key=lambda pair: amount - pair[0]["balance_due_minor"]
        )
        return AllocationOutcome(
            allocations=[
                InvoiceAllocation(
        ...
        for combo in itertools.combinations(invoices, size):
            ...
            for picks in itertools.product(*variant_lists):
                if sum(contrib for contrib, _ in picks) == amount:
                    ...
                    if variance_count:
                        reason += (
                            f" ({variance_count} with a TDS/fee variance absorbed)"
                        )
                    return AllocationOutcome(
                        allocations=allocations, match_type="SUBSET_SUM", reason=reason
                    )
    return AllocationOutcome()


async def partial_pay(
    payment: dict,
    bank_txn: dict,
    customer_id: str,
    ctx: AllocationContext,
